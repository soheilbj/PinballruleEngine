...
using PinballruleEngine.Parser;
using System;
...
namespace PinballruleEngine.Evaluator.EvaluationParser
{
    public class ExpressionEvaluatorExecutor
    {
          ...
          Dictionary<string, AbstractSyntaxTreeNode> symbolTable, DynamicBaseClass[] objectValues)
        {
            ...
            foreach (var item in postfixList)
            {
                ...
                else if (item is ObjectNode)
                {
                    ...
                    {
                        if (((ObjectNode)item).ObjectValue.Type == Enum.ObjectType.Collection)
                        {
        ...
        {
            StateMachineBuilder stateMaschineBuilder = new StateMachineBuilder();
            StateMachine stateMaschine = stateMaschineBuilder.Match(secondperatorString);
